...
void SI470X::getAllRegisters()
{
    ...
    Wire1.requestFrom(this->deviceAddress, 32);
    delayMicroseconds(300);
    // while (Wire1.available() < 32); // It did not work on Attiny Core
...
{
    word16_to_bytes aux;
    Wire1.beginTransmission(this->deviceAddress);
...
{
    word16_to_bytes aux;
    Wire1.requestFrom(this->deviceAddress, 2);
    delayMicroseconds(300);
    // while (Wire1.available() < 2) ; // It did not work on Attiny Core
 ...
 */
void SI470X::setup(int resetPin, int sdaPin, int sclPin, int rdsInterruptPin, int seekInterruptPin, uint8_t oscillator_type)
{
 ...
 * @param seekInterruptPin // optional. Sets the Arduino pin used to Seek function control. 
 */
void SI470X::setup(int resetPin, int sdaPin, int sclPin, uint8_t oscillator_type)
{
    setup(resetPin, sdaPin, sclPin, -1, -1, oscillator_type);
}
...
void SI470X::getRdsStatus()
{
    ...
    Wire1.requestFrom(this->deviceAddress, 12);
    delayMicroseconds(300);
    // while (Wire1.available() < 32); // It did not work on Attiny Core
    // The registers from 0x0A to 0x0F come first
    for (i = 0x0A; i <= 0x0F; i++)
 ...
 * @details For some reason, the BK1088 device does not work with the standard Wire1.h library of Arduino. 
 * @details The checkI2C function is only used to test the circuit. 
 * @details In practice, no function from the Wire1.h library is utilized in a real application with the BK1088 in this project. 
